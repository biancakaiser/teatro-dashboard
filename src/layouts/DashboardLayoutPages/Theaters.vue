	...
	<div class="md-layout">
		<div class="md-layout-item">
			<md-card>
				...
				<md-card-content>
					<md-table :value="queriedData" :md-sort.sync="currentSort" :md-sort-order.sync="currentSortOrder" :md-sort-fn="customSort"
				...
				<md-card-actions md-alignment="space-between">
					...
					<pagination class="pagination-no-border pagination-accent" v-model="pagination.currentPage" :per-page="pagination.perPage"
					 :total="total">
					</pagination>
				</md-card-actions>
			</md-card>
...
export default {
  ...
  methods: {
    ...
    openTheater: function(theater) {
      // this.$router.push(`/teatro/${theater.id}`);
      let routeData = this.$router.resolve(`/teatro/${theater.id}`);
      window.open(routeData.href, '_blank');
    },
    showSuccessSwal(message = "Sua resposta foi enviada.") {
      this.addTheaterModalHide();
      swal({
        title: "Sucesso",
        text: message,
        type: "success",
    ...
    showFailSwal(message = "Houve uma falha no servidor.") {
      ...
      swal({
        ...
        buttonsStyling: false,
        animation: false
      }).then(_ => (this.addTheaterModal = true));
    }
  },
  mounted() {
    this.loadTheaters();
